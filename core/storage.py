    ...
    if not has_unique_cv_job:
        ...
        INSERT OR IGNORE INTO match_results_new
            SELECT id, cv_id, job_id, rule_based_score, final_score, pros, cons,
                   recommendation, interview_questions, job_requirements_hash
            FROM match_results
            ORDER BY id DESC;
...
def save_match_result(cv_id, job_id, rule_based_score, final_score, pros, cons,
                       recommendation, interview_questions, job_requirements_hash=None):
    ...
    with _connection() as conn:
        conn.execute(
            ...
            (cv_id, job_id, rule_based_score, final_score, pros, cons, recommendation,
             interview_questions, job_requirements_hash)
            VALUES (?,?,?,?,?,?,?,?,?)
            ...
            (cv_id, job_id, rule_based_score, final_score, pros, cons, recommendation,
             json.dumps(interview_questions), job_requirements_hash),
        )
        conn.commit()
